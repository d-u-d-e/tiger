...
#define SHOULD_FAIL(filename, msgs)                                                                \
  ...
  TEST_CASE("invalid_book_examples")
  {
    ...
    parser.parse();
    CHECK(parser.had_error());
    CHECK(serr.str() == "[line 5:21] Err at 'nil': expected 'in' after let decls\n");
  }

  SHOULD_FAIL("empty.tig", "[line 1:1] Err at '$': expected expression\n");

  SHOULD_FAIL("junk.tig", "[line 7:1] Err at 'junk': unexpected token after expression\n");

  SHOULD_FAIL("seq.tig", "[line 4:4] Err at '$': expected ')' closing a sequence expression\n");
  ...
  SHOULD_FAIL("array.tig",
              ...
              "[line 5:15] Err at '[': expected type identifier before token '[' of "
              "array "
              "expression\n"
